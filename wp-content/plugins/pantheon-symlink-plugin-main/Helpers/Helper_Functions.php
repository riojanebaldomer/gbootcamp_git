...
class Helper_Functions
{
    ...
    public function create_symlink($target, $link)
    {
        ...
        $env = $this->is_pantheon_environment_writable();
        $env = $env['environment_check'];

        // get base path
        $base_path = $this->get_wp_homepath() . '/wp-content/uploads/';

        // Get the target folder name.
        if (preg_match('/\/uploads\/\W?\K.*/', empty($extension) ? $target : $directories, $matches)) {

            // check if folder does not exist create it
            if (!is_dir($base_path . $matches[0])) {
                ...
                if ($is_directory_created && empty($extension)) {
                    symlink($target, $link);
                } elseif ($is_directory_created && !empty($extension)) {
                ...
                //TODO: return status that directory exists, they may have to move the existing directory if it has content
            }
        }
    }
    ...
    {
        $filename = $this->get_wp_homepath() . '/wp-content/uploads/private/pantheon_symlink/pantheon_symlinks_config.json';
