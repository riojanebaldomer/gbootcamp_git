...
class Settings_Api implements Initializable_Interface, Model_Interface
{
    ...
    public function store_symlink($request)
    {
        ...
        // 2. save symlink on db and update symlink config file if creation of symlink is successful
        // 3.
        ...
        if ($is_new) {
            ...
            if ($success && !in_array($status, ['test', 'live'])) {
                // create symlink
                $this->_helper_functions->create_symlink($target, $link);
            }
        } else {

            if (!in_array($status, ['test', 'live'])) {
