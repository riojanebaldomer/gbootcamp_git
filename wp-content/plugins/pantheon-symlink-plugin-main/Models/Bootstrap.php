...
class Bootstrap implements Model_Interface
{
    ...
    private function _activate_plugin($blogid)
    {
        ...
        // initialize pantheon symlink config
        $this->_initialize_symlink_config();
    ...
    /**
     * Initialize symlink configuration file
     * - We will create a directory for symlink configuration file and create a symlink for it
     * this configuration file will be updated everytime Add, Update, and Delete is performed, this config file is
     * very useful for Test and Live environment simply because we cannot perform creation of symlinks due to not
     * writable, this config file will be executed in the Dev and Multi-dev environment to create the actual symlink
     * that is created in Test and Live environments.
     *
     * @return void
     * @since 2.1
     */
    private function _initialize_symlink_config()
    {
        $target = './uploads/private/pantheon_symlink/pantheon_symlinks_config.json';

        // Get target details
        $extension            = pathinfo($target, PATHINFO_EXTENSION);
        $filename             = pathinfo($target, PATHINFO_FILENAME);
        $directories          = pathinfo($target, PATHINFO_DIRNAME);
        $is_directory_created = false;
        $env                  = $this->_helper_functions->is_pantheon_environment_writable();
        $env                  = $env['environment_check'];

        // get base path
        $base_path = $this->_helper_functions->get_wp_homepath() . '/wp-content/uploads/';

        if (!in_array($env['environment'], ['test', 'live']) && $env['status'] === true) {

            if (preg_match('/\/uploads\/\W?\K.*/', $directories, $matches)) {
                $file_path = $base_path . $matches[0] . DIRECTORY_SEPARATOR . $filename . '.' . $extension;

                if (!is_dir($base_path . $matches[0])) {
                    $is_directory_created = mkdir($base_path . $matches[0], 0777, true);

                    if ($is_directory_created && !file_exists($file_path)) {
                        $data['symlinks'] = [];
                        $update_content   = json_encode($data, JSON_PRETTY_PRINT);
                        $success          = file_put_contents($file_path, $update_content);
                    }
